...
public class ShopScreen implements Listener {
	...
	public void itemScreen(Player player, int screenNo) {
		...
		itemScreen(player, screenNo, 1, i,true);
	}
	public void itemScreen(Player player, int screenNo, int pageNo) {
		Inventory i = plugin.getServer().createInventory(null, (54), itemTitle);
		itemScreen(player, screenNo, pageNo, i,true);
	}
	public void itemScreen(Player player, int screenNo, int pageNo, Inventory i, boolean First) {
	...
	public void amountScreen(Player player, Material mat, List<String> backMeta) {
		...
		back.setItemMeta(bM);
		double price = Shop.getPrice(new ItemStack(mat,1),State.QUERY);
